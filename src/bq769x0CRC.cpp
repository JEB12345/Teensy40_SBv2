...
{
  Wire.setSDA(18); // Use I2C0
  Wire.setSCL(19);
  Wire.setClock(100000); // Make sure clock is set to standard mode (100kHz)
  Wire.begin();
  // _wire = theWire;
...
void bq769x0::updateVoltages()
{
 ...
 Wire.requestFrom(I2CAddress, 2 * numberOfCells);

  idCellMaxVoltage = 0;
  idCellMinVoltage = 0;
  for (int i = 0; i < numberOfCells; i++)
  {
    adcVal = (Wire.read() << 8) | Wire.read();
...
void bq769x0::writeRegister(byte address, uint8_t data)
{
  ...
  Wire.write(address);
  Wire.write(data);
  Wire.write(CRC8.smbus(crcData,3));
  Wire.endTransmission();
}
...
{  
  ...
  Wire.write(address);
  Wire.endTransmission();
  Wire.requestFrom(I2CAddress, 2); // TODO: add CRC check for incoming data
  return Wire.read();
}
